				...
				@if(User::find(ChecklistController::id_user())->user_task()->get())
					...
					@if($category->id==$usertask->category)
						<tr id="{{$usertask->id}}">
							<td>
	  							@if($usertask->todo==0)
	  							<input type="checkbox"  onclick="clickCheckboxCat({{$category->id}},{{$usertask->id}})" id="chk_cat_{{$usertask->id}}" name="chk_checklist" value="{{$usertask->id}}"  />
								<input type="hidden"  onclick="clickCheckboxCat({{$category->id}},{{$usertask->id}})" name="checkbox-{{$usertask->id}}" value="">
	  							@else
		...
		var name_input = 'input[type="checkbox"]#chk_cat_'+idTask;
			...
			$.ajax({
				...
				success: function(data){
					...
					$("#Cat_overDue"+cat).text(data['Overdue']);
					$("#Cat_Completed"+cat).text(data['completed']);
					$("#warning"+idTask).replaceWith(data['waning']);
				}
